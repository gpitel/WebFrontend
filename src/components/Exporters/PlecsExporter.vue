...
export default {
    ...
    methods: {
        onClick() {
            this.exported = true;
            setTimeout(() => this.createPlecsSubcircuit(), 20);
            setTimeout(() => this.exported = false, 2000);
        },
        async createPlecsSubcircuit() {
            try {
                ...
                var subcircuit = await this.taskQueueStore.exportMagneticAsSubcircuit(magnetic, this.temperature, "PLECS", "");
                var blob = new Blob([subcircuit], { type: 'text/plain; charset=utf-8' });
                const filename = magnetic.manufacturerInfo.reference;
                download(blob, filename + ".plecs", "text/plain; charset=utf-8");
            } catch (error) {
                console.error(error);
            }
        },
...
<template>
    <div class="container">
        <button
            ...
            @click="onClick"
        >
            {{'Download magnetic subcircuit for PLECS'}}
        </button>
    </div>
</template>
